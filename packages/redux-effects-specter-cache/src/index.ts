import { Middleware, AnyAction, Store } from "redux";
...
function createCache(cacheOption?: LRUCache.Options<string, Record<string, any>>) {
...
export default function reduxEffectsSpecterCache<S = any>({
  ...
  return ({ getState }) => next => (action: Actions) => {
    ...
    const cacheKey = `@@$${SPECTER}/${service}@@${JSON.stringify(query)}`
