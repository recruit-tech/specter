...
export const unexpectedErrorHandler: ErrorHandler = [
  ...
  (err, _req, res) => {
    ...
    const error: { stack: string; name: string; message: string } =
      err instanceof Error ? { stack: err.stack, name: err.name, message: err.message } : err
    const payload: ErrorPayload = {
      ...error,
      handler: 'unexpectedErrorHandler',
      extendMessage: 'unhandled error in bff, you should implements a new ErrorHandler'
    }
    log.error(payload)
