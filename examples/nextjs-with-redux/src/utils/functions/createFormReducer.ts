...
export const createFormReducer = <S extends FormStateType<Record<string, any>, any>, V, P = any>(
  keyName: string,
  decideNextValue: (stateAndPayload: { state: S; payload: P }) => { state: S; payload: V },
  validator?: (args: S['form']['value']) => ValidationResultType
): ((state: S, payload: P) => S) => {
  return (state: S, payload: P) => {
    const nextValue = { ...state.form.value, [keyName]: decideNextValue({ state, payload }).payload }
    const nextForm = {
      value: nextValue,
      validationResults:
 ...
 * @param maxLength
 */
export const decideNextValueWithMaxLength = (maxLength: number) => ({ state, payload }: { state: any; payload: any }) => {
 ...
 * 半角数字以外は入力できない
 */
export const decideNextValueOnlyNumber = ({ state, payload }: { state: any; payload: any }) => {
