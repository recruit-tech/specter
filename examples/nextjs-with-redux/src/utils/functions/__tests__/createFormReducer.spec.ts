...
import {
  createFormReducer,
  decideNextValueWithMaxLength,
  decideNextValueOnlyNumber,
...
const INITIAL_STATE2: FormStateType<
  { test: string },
  { test: ValidationResultType }
> = {
  form: {
    value: { test: "" },
...
const requiredValidater = (v: { test: string }) =>
  required(() => "入力してください")(v.test);
  ...
  it("required validator: invalid", () => {
    ...
    assert.deepStrictEqual(nextState, {
      form: {
        value: { test: "" },
        validationResults: {
          test: { error: true, message: "入力してください" },
        },
      },
    });
  });
