...
import reduxEffectsUploader, { Config as UploaderConfig } from 'redux-effects-formdata-uploader'
...
const cacheMiddlewareConfig = isServer
  ? {
      toCache: (action: AnyAction) => action.payload.service.startsWith('master.')
    }
  : {
...
function createStore(initialState: RootState) {
  ...
  const middlewares = [
    ...
    reduxEffectsSteps,
    reduxEffectsSpecterCache({ cacheOption, middlewareOption: cacheMiddlewareConfig }),
    reduxEffectsSpecter(client),
    isClient && reduxEffectsUploader(uploaderConfig)
  ].filter(Boolean)

  const store = createReduxStore(reducer, initialState, enhancedCompose(applyMiddleware(...middlewares)))
