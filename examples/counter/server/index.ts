...
app.use(
  "/public",
  express.static(path.resolve(process.cwd(), "./webpack/dist/client"))
);
...
app.get("/ssr", async (req, res) => {
  ...
  res.send(
    `<html><head><meta id="initial-data" data-count="${count}" /></head><body><div id="root">${str}</div><script src="/public/bundle.js"></script></body></html>`
  );
});
